...
# Download and GEO data 

options(timeout = 600)
options(download.file.method = "libcurl")
gset<-getGEO("GSE37182",GSEMatrix =TRUE, AnnotGPL=TRUE)
...
gsms<-paste0(c(rep(0,88),rep(1,84)))  #0 for nomal, 1 for CRC
sml <- factor(gsms, levels = c("0", "1"), labels = c("Normal", "CRC"))

# Verify the grouping matches your samples
table(sml)  # Should show 88 Normal and 84 CRC
...
pheatmap(
  ...
  show_rownames = TRUE,
  show_colnames = FALSE,  # Often cleaner with many samples
  cluster_rows = TRUE,
  cluster_cols = TRUE,
  fontsize_row = 10,
...
egfr_expr <- exprs(gset)[which(fData(gset)$Gene.symbol == "PIK3CA"), ]
